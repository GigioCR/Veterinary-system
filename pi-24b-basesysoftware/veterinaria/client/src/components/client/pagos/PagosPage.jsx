...
export default function PagosPage () {
  ...
  const fetchPagos = async () => {
    try {
      const response = await axiosInstance.get(`http://localhost:8080/clientes/pagos/${localStorage.getItem('userId')}`);
  ...
  const fetchCitasServicios = async () => {
    try {
      const response = await axiosInstance.get(`http://localhost:8080/clientes/pagos/servicios/${localStorage.getItem('userId')}`);
  ...
  const handleClickPagar = () => {
    ...
    navigate(`${selectedPago.ID_PAGO}`, {
      state: {
        ID_PAGO: selectedPago.ID_PAGO,
        ID_CITA: selectedPago.ID_CITA,
        MONTO: selectedPago.MONTO,
        CITAS_SERVICIOS: citasServicios.filter(servicio => servicio.ID_CITA == selectedPago.ID_CITA)
      }
    });
  };
  ...
  const filteredPagosPendientes = pagosPendientes.filter((p) => {
    ...
    return fecha.includes(search) || hora.includes(search) || mascota.includes(search) || veterinario.includes(search);
  });
            ...
            <TabContext value={tabValue}>
              ...
              {isSmallScreen && tabValue ==="1" ||  isMediumScreen && tabValue === "1" ? (
                <Box>
                  ...
                  ? <Typography marginTop={2} marginLeft={1}>No tiene pagos pendientes.</Typography> 
                  : <div>
                          ...
                          {pagosPendientes.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((pago) => (
                              <Box key={pago.ID_PAGO} sx={{ border: '1px solid #ccc', padding: 2, borderRadius: 2, position:'relative' }}>
                                  ...
                                  >
                                    ...
                                    <MenuItem onClick={handleClickPagar}>Pagar</MenuItem>
                                  </Menu>
                              </Box>
                ...
                <TabPanel value="1">
                            ...
                            <TableBody>
                              ...
                              .map((pago) => (
                                <TableRow key={pago.ID_PAGO}>
                                  ...
                                  >
                                    <MenuItem onClick={handleOpenPagoVer}>Detalles</MenuItem>
                                    <MenuItem onClick={handleClickPagar}>Pagar</MenuItem>
                                  </Menu>
                                </TableRow>
                              ))}
                  ...
                  : // render in case of no data to display
                    (<Typography>No tiene pagos pendientes.</Typography>)
                  }
                </TabPanel>
              }
              ...
              {isSmallScreen && tabValue ==="2" ||  isMediumScreen && tabValue === "2"  ? (
                <Box>
                  ...
                  ? <Typography marginTop={2} marginLeft={1}>No tiene pagos pasados.</Typography> 
                  : <div>
                      <Box sx={{ display: 'grid', paddingLeft:{xs: 0, sm: 3, md: 3, lg: 3},  gridTemplateColumns: {xs: 'repeat(1, 1fr)', sm:'repeat(3, 3fr)',  md:'repeat(3, 3fr)'}, gap: {xs: 2, sm: 4, md: 4}, marginTop: 2 }}>
                ...
                <TabPanel value="2">
                  {(pagosPasados.length > 0) ? (
                    ...
                    <Typography>No tiene pagos pasados.</Typography>
                  )}
                </TabPanel>
              }
            </TabContext>

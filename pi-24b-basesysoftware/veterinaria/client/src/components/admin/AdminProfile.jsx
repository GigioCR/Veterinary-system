...
export default function AdminProfile() {
  ...
  const fetchInfo = async () => {
    try {
      await axiosInstance.get(`http://localhost:8080/administradores/perfil/${localStorage.getItem('userId')}`
    ).then((res) => {
  ...
  const handleSubmitPasswordChange = async (e) => {
    ...
    await axiosInstance
    .put(`http://localhost:8080/administradores/perfil/contrasena/check/${localStorage.getItem('userId')}`, userPassword)
    .then((response) => {
    ...
    if (Object.keys(newErrores).length > 0) {
      ...
      await axiosInstance
      .put(`http://localhost:8080/administradores/perfil/contrasena/${localStorage.getItem('userId')}`, userPassword)
      .then((response) => {
        console.log(response);
  ...
  const handleSubmitUserData = async (e) => {
    ...
    if (Object.keys(newErrores).length > 0) {
      ...
      await axiosInstance
        .put(`http://localhost:8080/administradores/perfil/datos/${localStorage.getItem('userId')}`, userData)
        .then((response) => {
          console.log(response);
          setRefresh(prevRefresh => prevRefresh + 1 );
          setErrores("")

...
export default function AdminDashboard() {
  ...
  const fetchInfo = async () => {
    try {
      await axiosInstance.get('http://localhost:8080/administradores/').then((res) => {
        const dataReformatted = res.data.map(dataArray => ({
          SERVICIOS: dataArray[0],
          VETERINARIOS: dataArray[1],
          CLIENTES: dataArray[2],
          MASCOTAS: dataArray[3]
        }))
        setData(dataReformatted)
        console.log(dataReformatted)
      })
      await axiosInstance.get('http://localhost:8080/administradores/calendario').then((res) => {
            ...
            <Grid container spacing={{ xs: 2, md: 3 }} columns={{ xs: 1, md: 2 }} alignContent='start'>
                ...
                <Item>      
                  <CardContent>
                    <Typography gutterBottom variant="h5" component="div" align='center' color='black'>
                      {data.length != 0 ? data[0].VETERINARIOS : ""} Veterinarios
                    </Typography>
                  </CardContent>
                </Item>
              </Grid>
              <Grid size={1}>
                <Item>      
                  <CardContent>
                    <Typography gutterBottom variant="h5" component="div" align='center' color='black'>
                      {data.length != 0 ? data[0].CLIENTES : ""} Clientes
                    </Typography>
                  </CardContent>
                </Item>
              </Grid>
              ...
              <Grid size={1}>
                <Item>      
                  <CardContent>
                    <Typography gutterBottom variant="h5" component="div" align='center' color='black'>
                      {data.length != 0 ? data[0].SERVICIOS : ""} Servicios
                    </Typography>
                  </CardContent>
                </Item>
              </Grid>
            ...
            <Box sx={style}>
              ...
              {citas.map((cita) => (
                (cita.FECHA === selectedDate) && (cita.ESTADO === 1)
                  ? 
                    (<Card key={cita.ID_CITA} sx={{ minWidth: 275, marginBottom:'15px', marginTop:'15px' }}>
                      <CardContent>

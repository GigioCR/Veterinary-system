...
const style = {
    ...
    const AddMascot = ({open, onClose, handleModificationSuccess}) => {
        ...
        const HandleChange = (e, newValue) => {
            console.log(JSON.stringify(filteredBreeds))
                if (e.target.name) {
        ...
        const fetchAvailableClients = async() => {
            try {
                const response = await axiosInstance.get("http://localhost:8080/veterinarios/availableClients");
        ...
        const fetchSpecies = async() => {
            try {
                const response = await axiosInstance.get("http://localhost:8080/veterinarios/especies");
        ...
        const fetchBreeds = async() => {
            try {
                const response = await axiosInstance.get("http://localhost:8080/veterinarios/razas");
        ...
        //   // useEffect to filter breeds based on selected species
        //     useEffect(() => {
        //         const filtered = breeds.filter(breed => breed.SPECIES_ID === selectedSpecies);
        //         setFilteredBreeds(filtered);
        //     }, [selectedSpecies, breeds]); // Re-run when selectedSpecies or breeds change
        ...
        const handleClickAdd = async e => {
            ...
            if (mascot.USR_ID.length <= 0) {
                newErrores.USER_ID = true;
            }
            ...
            try {
                console.log("la mascota a agregar es: " + JSON.stringify(mascot))
                const result = await axiosInstance.post("http://localhost:8080/veterinarios/mascotas/add", mascot)
                ...
                >
                    ...
                    <Box display="flex" justifyContent="space-between" alignItems="center">
                        ...
                        <Autocomplete
                            ...
                            renderInput={(params) => <TextField required {...params} label="Cliente" name='USR_ID' error = {errores.USER_ID}
                            helperText = {errores.USER_ID? "Debe elegir un cliente":""} InputLabelProps={inputPropsStyle} value={mascot.USR_ID} />}
                        />

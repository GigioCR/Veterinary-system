...
export default function VetProfile() {
  ...
  const fetchInfo = async () => {
    try {
      await axiosInstance.get(`http://localhost:8080/veterinarios/perfil/${localStorage.getItem('userId')}`
    ).then((res) => {
  ...
  const handleSubmitPasswordChange = async (e) => {
    ...
    await axiosInstance
    .put(`http://localhost:8080/veterinarios/perfil/contrasena/check/${localStorage.getItem('userId')}`, userPassword)
    .then((response) => {
    ...
    if (Object.keys(newErrores).length > 0) {
      ...
      await axiosInstance
        .put(`http://localhost:8080/veterinarios/perfil/contrasena/${localStorage.getItem('userId')}`, userPassword)
        .then((response) => {
          console.log(response);
  ...
  const handleSubmitUserData = async (e) => {
    ...
    if (Object.keys(newErrores).length > 0) {
      ...
      await axiosInstance
        .put(`http://localhost:8080/veterinarios/perfil/datos/${localStorage.getItem('userId')}`, userData)
        .then((response) => {
          console.log(response);
          setRefresh(prevRefresh => prevRefresh + 1 );
          setErrores("")
          ...
          <Box className='dashSection' sx={{ width: '70%', marginLeft: '35px' }}>
            ...
            {data.map((d) => (
              <FormGroup key={d.ID}>
                ...
                />
                <TextField label="Especialidad" defaultValue={d.ESPECIALIDAD} variant="outlined" fullWidth margin="normal"
                  disabled
                  sx={{ maxWidth: '50ch', 
                    '& .MuiOutlinedInput-root.Mui-disabled': {
                      backgroundColor: 'white', // Change background color
                ...
                />
                <TextField label="Salario" defaultValue={'₡' + d.SALARIO} variant="outlined" fullWidth margin="normal"
                  disabled
                  sx={{ maxWidth: '50ch', 
                    '& .MuiOutlinedInput-root.Mui-disabled': {
                      backgroundColor: 'white', // Change background color
